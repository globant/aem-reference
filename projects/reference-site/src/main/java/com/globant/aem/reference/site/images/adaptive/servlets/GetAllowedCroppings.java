...
public class GetAllowedCroppings extends SlingSafeMethodsServlet {
  ...
  protected void doGet(SlingHttpServletRequest request, final SlingHttpServletResponse response)
    ...
    if (resource != null) {
      for (Resource child : resource.getChildren()) {
        ...
        if (croppingRect != null) {
          ...
          String heigth = croppingRect.get("h", String.class);
          croppings.put(child.getName(), coordX + "," + coordY + "," + width + "," + heigth + "/");
        }
      }

    }

    AdaptiveImageService adaptiveImageService = serviceFacade.getServiceReference(path);
    ...
    try {
      ...
      if (adaptiveImageService != null && adaptiveImageService.getAllowedCroppingsArray() != null) {
        List<String> allowedCroppings = adaptiveImageService.getAllowedCroppingsArray();
